                ...
                <form class="flex flex-wrap items-center gap-3" action="/admin" method="get">
                    ...
                    <div class="min-w-[100px]">
                        <select name="gender"
                            ...
                            <option value="" {{ request('gender') == '' ? 'selected' : '' }}>
                                性別
                            </option>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($contacts as $contact)
                            <tr>
                                <td class="px-6 py-4 text-sm text-gray-700">{{ $contact->first_name }}
                                ...
                                <td class="px-6 py-4 text-sm text-gray-700">
                                    @if(method_exists($contact, 'tags'))
                                        @foreach ($contact->tags as $tag)
                                            <span
                                                class="inline-block bg-gray-100 text-gray-700 text-xs px-2 py-1 rounded mr-1">{{ $tag->name }}</span>
                                        @endforeach
                                    @endif
                                </td>
